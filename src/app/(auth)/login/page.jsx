...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (storedLoggedIn === 'true') {
      setIsLoggedIn(true); // ✅ Track local storage in state
      router.push('/');
    }
  }, [router]);
  ...
  const handleLogin = (e) => {
    ...
    if (email === 'ananfack@gmail.com' && password === 'password123') {
      ...
      setIsLoggedIn(true); // ✅ Set after login
      router.push('/');
    } else {
  ...
  // ✅ Optional: render success message if already logged in
  if (isLoggedIn) {
    return (
        ...
        <div className="flex w-full max-w-6xl items-center gap-12">
          <div className="flex-1">
            <div className="max-w-lg">
              ...
              {message && (
                <div className={`p-3 rounded mb-4 ${message.includes('Welcome') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                  {message}
                </div>
              )}
